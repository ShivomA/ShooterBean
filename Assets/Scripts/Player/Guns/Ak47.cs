...
public class Ak47 : MonoBehaviour
{
    ...
    float reloadingTime = 3.5f, zoomFactor = 1.25f;
    int bulletCount, maxBullet = 30, firingRate = 175, muzzleVelocity = 500;
    int damage = 30, criticalDamage = 60;

    float xSpread = 0.004f, ySpread = 0.01f, zSpread = 0, gunAngle = 0;
    float spreadAngleX = 0, spreadAngleY = 0, spreadAngleZ = 0;
    float recoilAngle = -5, recoilStep = -0.5f, maxRecoil = 45;
    int recoilDelay = 20;
    ...
    {
        if (!reset)
            myTransform.localPosition = new Vector3(0, -1.12f, 0.3f);
        if (reset)
            myTransform.localPosition = new Vector3(0.65f, -1.04f, 0.46f);
