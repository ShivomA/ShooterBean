...
public class SMG : MonoBehaviour
{
    ...
    float reloadingTime = 2, zoomFactor = 1.5f;
    int bulletCount, maxBullet = 30, firingRate = 100, muzzleVelocity = 250;
    int damage = 20, criticalDamage = 35;

    float xSpread = 0.002f, ySpread = 0.005f, zSpread = 0, gunAngle = 0;
    float spreadAngleX = 0, spreadAngleY = 0, spreadAngleZ = 0;
    float recoilAngle = -5, recoilStep = -0.5f, maxRecoil = 20;
    int recoilDelay = 20;
    ...
    {
        if (!reset)
            myTransform.localPosition = new Vector3(0, -1, 1.5f);
        if (reset)
            myTransform.localPosition = new Vector3(0.72f, -1.1f, 1.55f);
